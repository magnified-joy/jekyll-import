...
WHERE pm.post_id = posts.id AND pm.meta_key = '_thumbnail_id' AND pm2.post_ID = pm.meta_value AND pm2.meta_key = '_wp_attached_file'"
        ...
        if options[:status] and not options[:status].empty?
          ...
          posts_query << "
           AND posts.post_status = '#{status.to_s}'"
      ...
      def self.process_post(post, db, options, page_name_list)
		...
		elsif (categories[0] and categories[0].downcase.include? "wedding") or post[:type].to_s.downcase == 'wedding'
			filename = "wedding/_posts/#{name}"
		elsif (categories[0] and categories[0].downcase.include? "engagement") or post[:type].to_s.downcase == 'engagement'
			filename = "engagement/_posts/#{name}"
